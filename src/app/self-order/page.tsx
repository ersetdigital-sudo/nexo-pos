...
export default function SelfOrderPage() {
  ...
  if (orderPlaced) {
    return (
      <div className="min-h-screen bg-surface-200 flex items-center justify-center p-6">
        <div className="bento-card-lg text-center max-w-md w-full animate-slide-up">
          ...
          <h1 className="text-2xl font-bold text-text mb-2">Pesanan Diterima!</h1>
          <div className="text-4xl font-bold text-primary-600 my-4">#{orderNumber}</div>
          <p className="text-text-muted mb-6">Pesanan Anda sedang diproses. Tunggu nomor antrian dipanggil.</p>
  ...
  return (
    <div className="min-h-screen bg-surface-200">
      {/* Header */}
      <div className="bg-white border-b border-primary-100/60 p-4 sticky top-0 z-30 shadow-bento">
      ...
      {/* Categories */}
      <div className="sticky top-[65px] z-20 bg-white border-b border-primary-100/40 p-3 overflow-x-auto">
      ...
      <div className="max-w-4xl mx-auto p-4">
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-3">
          {filteredProducts.map((product) => (
            <div key={product.id} className="bento-card flex gap-3 items-center">
              <div className="w-12 h-12 rounded-xl bg-primary-50 border border-primary-100/40 flex items-center justify-center text-2xl flex-shrink-0">
                {product.image}
              </div>
              <div className="flex-1 min-w-0">
                <h3 className="font-semibold text-sm text-text truncate">{product.name}</h3>
              ...
              <button onClick={() => addToLocalCart(product)}
                className="w-8 h-8 rounded-lg bg-primary-200 flex items-center justify-center flex-shrink-0 hover:bg-primary-300 transition-colors">
      ...
      {/* Bottom bar */}
      {cart.length > 0 && (
        <div className="fixed bottom-0 left-0 right-0 bg-white border-t border-primary-100/60 p-4 z-30 shadow-bento-md">
          <div className="max-w-4xl mx-auto flex items-center justify-between">
            <div>
              <p className="text-sm font-medium text-text-secondary">{cart.reduce((s, i) => s + i.quantity, 0)} item</p>
      ...
      {showCart && (
        <div className="fixed inset-0 bg-black/40 flex items-end sm:items-center justify-center z-50 animate-fade-in">
          <div className="bento-card-lg w-full max-w-md max-h-[90vh] overflow-y-auto rounded-b-none sm:rounded-b-2xl">
            <div className="flex justify-between items-center mb-4">
              <h2 className="text-lg font-bold text-text">Pesanan Anda</h2>
              <button onClick={() => setShowCart(false)} className="p-2 rounded-lg hover:bg-primary-50">
            ...
            {cart.length === 0 ? (
              <div className="text-center py-10">
                ...
                <p className="text-text-muted">Keranjang kosong</p>
              </div>
            ) : (
              <>
                <div className="space-y-3 mb-4">
                  {cart.map((item) => (
                    <div key={item.id} className="flex items-center justify-between p-3 rounded-xl bg-primary-50 border border-primary-100/40">
                      <div className="flex items-center gap-2">
                        <span className="text-xl">{item.product.image}</span>
                        <div>
                          <p className="font-medium text-sm text-text">{item.product.name}</p>

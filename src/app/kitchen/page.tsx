    ...
    <div className="min-h-screen bg-gray-900 p-6">
      ...
      <div className="flex items-center justify-between mb-6">
        <div className="flex items-center gap-3">
          ...
          <div>
            ...
            <p className="text-sm text-gray-400">{kitchenOrders.length} pesanan aktif</p>
          </div>
        </div>
      ...
      {kitchenOrders.length === 0 ? (
        <div className="text-center py-32">
          ...
          <p className="text-2xl font-bold text-gray-300">Semua Selesai!</p>
          <p className="text-gray-500 mt-2">Menunggu pesanan baru masuk...</p>
        </div>
      ) : (
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-4">
          {kitchenOrders.map((order) => (
            <div key={order.id}
              ...
              {order.tableNumber && (
                <div className="text-sm font-semibold text-primary-200 mb-3">Meja {order.tableNumber}</div>
              )}

              <div className="space-y-2 mb-5">
                {order.items.map((item, i) => (
                  <div key={i} className="p-3 rounded-lg bg-gray-900/60 border border-gray-700/50">
                    <div className="flex justify-between items-center">
                      <span className="font-medium text-white">{item.product.name}</span>
                      <span className="text-xl font-bold text-primary-200">x{item.quantity}</span>
                    </div>
              ...
              {order.status === "pending" ? (
                <button onClick={() => updateOrderStatus(order.id, "preparing")}
                  className="w-full py-3 rounded-lg bg-amber-500 text-white font-semibold text-sm hover:bg-amber-600 transition-colors shadow-bento">
                  Mulai Masak
                </button>
              ) : (
                <button onClick={() => updateOrderStatus(order.id, "ready")}
                  className="w-full py-3 rounded-lg bg-emerald-500 text-white font-semibold text-sm hover:bg-emerald-600 transition-colors shadow-bento">
                  Selesai
                </button>
              )}
            </div>
